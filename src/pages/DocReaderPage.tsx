...
export const DocReaderPage: React.FC = () => {
  ...
  // Image display accessibility controls
  const [isHighContrastImage, setIsHighContrastImage] = useState<boolean>(false);
  ...
  // Helper to create high contrast rendered fallback document image if camera frame was dark/pitch black
  const createFallbackDocumentCanvas = useCallback((docType: string, textSnippet: string): string => {
    ...
    ctx.fillRect(48, 48, 1184, 88);

    ctx.fillStyle = '#000000';
    ctx.font = 'bold 36px sans-serif';
    ctx.fillText('DOCUMENT SCANNER • VISION ASSIST', 80, 105);

    ctx.fillStyle = '#0f172a';
    ctx.font = 'bold 28px sans-serif';
    ctx.fillText(`TYPE: ${(docType || 'Exam Paper / Document').toUpperCase()}`, 80, 195);

    ctx.fillStyle = '#334155';
    ctx.font = '22px monospace';
    const lines = (textSnippet || 'Captured document text sample').split('\n').flatMap((l) => {
      const chunks = [];
      for (let i = 0; i < l.length; i += 65) {
        chunks.push(l.slice(i, i + 65));
      }
  ...
  const captureDocument = useCallback(async () => {
    ...
    let imageBase64 = canvas.toDataURL('image/jpeg', 0.88);

    // Sample pixels to verify if captured camera frame is pitch black
    ...
    const avgBrightness = totalBrightness / (imgData.data.length / 16);
    const isFrameDark = avgBrightness < 12;
    ...
    try {
      ...
      if (!finalText || finalText.length < 3) {
        ...
        await speak(
          "I couldn't read any text clearly from this image. Please hold the document closer and ensure good lighting."
        );
        initCamera('document');
        return;
      }

      // If video frame was dark/pitch black, generate a rendered high-contrast preview canvas
      if (isFrameDark) {
        const fallbackCanvasImg = createFallbackDocumentCanvas(finalType, finalText);
        setCapturedImage(fallbackCanvasImg);
      }
  ...
  }, [speak, speakChunks, setStatusMessage, stopCamera, initCamera, createFallbackDocumentCanvas]);
      ...
      <section className="bg-zinc-900 border-4 border-zinc-700 hover:border-yellow-500 rounded-[32px] p-6 sm:p-10 space-y-8 shadow-[0_10px_40px_rgba(0,0,0,0.6)] transition-all">
        ...
        {(isProcessingOCR || isSummarizing) && (
          <div className="p-8 bg-zinc-950 border-2 border-yellow-500/50 rounded-3xl text-center space-y-6 shadow-inner">
            ...
            {capturedImage && (
              <div className="max-w-md mx-auto rounded-2xl overflow-hidden border-2 border-yellow-500/60 shadow-xl bg-zinc-900 p-3">
                <img src={capturedImage} alt="Captured Document" className="w-full object-contain max-h-64 rounded-xl" />
              </div>
            )}
          </div>
        )}
        ...
        {mode === 'RESULT' && (
          <div className="space-y-6">
            ...
            {capturedImage && (
              <div className="p-6 bg-zinc-950 border-2 border-yellow-500/70 rounded-3xl space-y-4 shadow-xl">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800 pb-3">
                  <div className="flex items-center gap-2 text-yellow-400 font-black text-base sm:text-lg uppercase tracking-wider">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      onClick={() => setIsHighContrastImage((prev) => !prev)}
                      className={`px-3 py-1.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all border ${
                        isHighContrastImage
                          ? 'bg-yellow-500 text-black border-yellow-400 font-black'
                          : 'bg-zinc-800 text-zinc-300 border-zinc-700 hover:bg-zinc-700'
                      }`}
                    >
                      {isHighContrastImage ? 'High Contrast On' : 'Invert / High Contrast'}
                    </button>

                    <button
                      onClick={() => setIsImageExpanded((prev) => !prev)}
                      className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl border border-zinc-700 flex items-center gap-1.5 cursor-pointer"
                    >
                      {isImageExpanded ? (
                        <>
                ...
                <div className="p-3 bg-zinc-900 border border-zinc-800 rounded-2xl flex items-center justify-center overflow-hidden">
                  <div
                    className={`relative w-full max-w-2xl rounded-xl overflow-hidden border border-zinc-700 bg-zinc-950 transition-all ${
                      isHighContrastImage ? 'invert contrast-200' : ''
                    }`}
                  >
                    <img
                      src={capturedImage}
                      ...
                      className={`w-full object-contain mx-auto rounded-lg ${
                        isImageExpanded ? 'max-h-[80vh]' : 'max-h-80'
                      }`}
                    />
                  </div>
                </div>
        ...
        <div className="pt-4 border-t-2 border-zinc-800 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button
              ...
              className="p-6 bg-yellow-500 hover:bg-yellow-400 text-black font-black text-lg sm:text-xl uppercase tracking-wider rounded-2xl transition-all shadow-xl flex items-center justify-center gap-3 cursor-pointer"
            >
              <Camera className="w-7 h-7 stroke-[3]" />
              <span>Scan Document</span>
            </button>

            <button
              onClick={() => initCamera('code')}
              className="p-6 bg-zinc-800 hover:bg-zinc-700 text-white font-black text-lg sm:text-xl uppercase tracking-wider rounded-2xl transition-all border-2 border-zinc-600 flex items-center justify-center gap-3 cursor-pointer"
            >
              <QrCode className="w-7 h-7 text-yellow-500 stroke-[2.5]" />
              <span>Scan Code</span>
            </button>
          </div>
