...
import { Mic, MicOff, Volume2, AlertTriangle, Radio, RefreshCw } from 'lucide-react';
...
export const VoiceStatusCard: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-800/80 pb-4">
        <div className="flex items-center space-x-3.5">
          ...
          <div>
            ...
            <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tight flex items-center gap-2 mt-0.5">
              {isSpeaking ? (
                <span className="text-yellow-400">System Speaking...</span>
              ) : isListening ? (
                <span className="text-white">System Listening</span>
              ) : (
                <span className="text-zinc-400">System Paused</span>
              )}
            </h2>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>Test Speaker Voice</span>
          </button>

          <button
            onClick={toggleListening}
            className={`px-5 py-2.5 rounded-xl text-xs sm:text-sm font-black uppercase tracking-wider border-2 transition-all focus:ring-4 focus:ring-yellow-500 ${
              isListening
                ? 'bg-zinc-800 border-zinc-700 text-zinc-200 hover:bg-zinc-700'
                : 'bg-yellow-500 border-yellow-400 text-black hover:bg-yellow-400 shadow-[0_0_15px_rgba(234,179,8,0.3)]'
            }`}
          >
            {isListening ? 'Pause Voice System' : 'Activate Voice System'}
          </button>
        </div>
      </div>
      ...
      <div className="w-full bg-[#0a0a0a] rounded-2xl border-2 border-zinc-800 p-6 sm:p-8 flex flex-col items-center justify-center relative overflow-hidden min-h-[160px] sm:min-h-[200px]">
        ...
        <div className="z-10 text-center space-y-1">
          ...
          {(transcript || interimTranscript) && (
            <p className="text-yellow-400 font-mono text-base sm:text-xl font-bold bg-zinc-900/90 px-4 py-2 rounded-xl border border-yellow-500/40 inline-block mt-2">
              "{transcript || interimTranscript}"
            </p>
          )}
        </div>
      ...
      {/* Audio Policy Prompt if User Interaction Needed */}
      {!audioUnlocked && (
        <button
          onClick={unlockAudio}
          className="w-full py-4 px-6 bg-yellow-500 text-black font-black text-sm uppercase tracking-wider rounded-xl hover:bg-yellow-400 transition-colors flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(234,179,8,0.3)] border border-yellow-400 cursor-pointer"
        >
          <Volume2 className="w-5 h-5 stroke-[2.5]" />
          <span>Tap Here to Enable Audio Voice Output</span>
        </button>
      )}

      {/* Browser Support / Mic Permission Error Fallback Warning */}
      {(!isSupported || micError) && (
        <div className="bg-zinc-950 border-2 border-rose-500/80 text-rose-200 rounded-2xl p-5 flex items-start space-x-4">
          ...
          <div className="space-y-1 text-sm">
            ...
            <p className="text-zinc-300 font-medium">
              {micError ||
                'Your browser does not support Web Speech Recognition. You can use the high-contrast buttons below to navigate.'}
            </p>
          </div>
        </div>
      )}
