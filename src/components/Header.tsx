...
import { Mic, MicOff, Volume2, HelpCircle, Eye, ArrowLeft } from 'lucide-react';

export const Header: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { isListening, isSpeaking, toggleListening, speakHelp, executeCommand } = useVoice();
      ...
      <div className="max-w-5xl mx-auto flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="flex items-center space-x-2 px-5 py-2.5 bg-white text-black hover:bg-zinc-200 font-black rounded-xl text-sm uppercase transition-colors focus:outline-none focus:ring-4 focus:ring-yellow-500 shadow-lg cursor-pointer"
          >
            <HelpCircle className="w-4 h-4 text-black stroke-[3]" />
            <span className="font-black">Help</span>
          </button>

          {/* Global Back Navigation Button (placed near and to the right of Help button) */}
          <button
            onClick={handleGoBack}
            aria-label="Go to previous page"
            className="flex items-center space-x-2 px-5 py-2.5 bg-yellow-500 hover:bg-yellow-400 text-black font-black rounded-xl text-sm uppercase transition-colors focus:outline-none focus:ring-4 focus:ring-yellow-500 shadow-lg cursor-pointer"
          >
